...
def get_count_brows(li, browser):
	count = 0
	length = len(li)
	for i in li:
		string = str(i)
...
def get_count_sbrows(li, browser):
	count = 0
	length = len(li)
	for i in li:
		string = str(i)
...
def create_dict_uuid_brows(browsers, li):
	dict = {}
	length = len(browsers)
	for i in range(length):
		browser = browsers[i]
		count = get_count_brows(li, browser)
		dict.update({browser : count})
	return (dict)

def create_dict_uuid_sbrows(sbrowsers, li):
	dict = {}
	length = len(sbrowsers)
	for i in range(length):
		browser = sbrowsers[i]
		count = get_count_sbrows(li, browser)
		dict.update({browser : count})
	return (dict)
